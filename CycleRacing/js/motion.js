/*jslint plusplus: true, sloppy: true, indent: 4 */
 ...
 var canvas = null,
  ...
  function loadBackground() {
    ...
    background.src = 'img/race-scence.png';
 ...
 function setupGameData() {
  ...
  function initCar(current_car) {  
    ...
    console.log(Math.floor(Math.random()* game_data.graphs.length));
  }
  ...
  function drawText(current_car) {
    if(current_car.position !== null) {
        ...
        ctx.strokeText(current_car.position, RACE_FINISH_LINE_X + 
		   current_car.step + 110, current_car.y + 25);
    }
  }
  function moveCar(iCarCounter) {
    ...
    if(current_car.x < RACE_FINISH_LINE_X) {
        current_car.x = 
          graphPosToScreenPos(position) + (velocity * seconds) + (1/2 * a * Math.pow(seconds, 2));
    }
    else {
  ...
  function loadCarSprite() {
    ...
    car_sprite.src = 'img/car-sprite.gif';
